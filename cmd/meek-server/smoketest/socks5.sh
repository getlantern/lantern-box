...
# Per-run private temp dir — avoids cross-run collisions and symlink
# races that fixed /tmp paths invite on shared hosts.
WORKDIR=$(mktemp -d "${TMPDIR:-/tmp}/meek-smoke.XXXXXX")
...
open('${WORKDIR}/p1-methodsel.bin', 'wb').write(b'\x05\x01\x00')

# CONNECT httpbin.org:80
buf = bytearray(b'\x05\x01\x00\x03')
buf += bytes([len(target_host)]) + target_host
buf += target_port.to_bytes(2, 'big')
open('${WORKDIR}/p2-connect.bin', 'wb').write(bytes(buf))

# HTTP request the proxy forwards once CONNECT completes
http = b'GET /ip HTTP/1.0\r\nHost: ' + target_host + b'\r\nConnection: close\r\n\r\n'
open('${WORKDIR}/p3-http.bin', 'wb').write(http)
PYEOF

echo ""
echo "--- Phase 1: SOCKS5 method-select (NO_AUTH) ---"
n=$(send_and_drain ${WORKDIR}/p1-methodsel.bin ${WORKDIR}/r1.bin 2 8)
echo "received ${n} bytes: $(xxd -p ${WORKDIR}/r1.bin | head -1)"
if ! [ "$(xxd -p ${WORKDIR}/r1.bin | head -c 4)" = "0500" ]; then
...
n=$(send_and_drain ${WORKDIR}/p2-connect.bin ${WORKDIR}/r2.bin 10 8)
echo "received ${n} bytes: $(xxd -p ${WORKDIR}/r2.bin | head -1)"
# SOCKS5 CONNECT reply: 05 00 00 01 ... (success)
if ! [[ "$(xxd -p ${WORKDIR}/r2.bin | head -c 8)" =~ ^050000 ]]; then
...
echo ""
echo "--- success check ---"
if grep -q '"origin"' ${WORKDIR}/r3.bin; then
  origin=$(grep -o '"origin"[^}]*' ${WORKDIR}/r3.bin)
